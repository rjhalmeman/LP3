...
JLabel labelIdProduto = new JLabel("IdProduto");JTextField textFieldIdProduto = new JTextField(20);
JLabel labelDataCadastro = new JLabel("DataCadastro");JTextField textFieldDataCadastro = new JTextField(20);
JLabel labelNomeProduto = new JLabel("NomeProduto");JTextField textFieldNomeProduto = new JTextField(20);
JLabel labelPrecoUnitarioProduto = new JLabel("PrecoUnitarioProduto");JTextField textFieldPrecoUnitarioProduto = new JTextField(20);
JLabel labelQuantidadeMinimaEstoque = new JLabel("QuantidadeMinimaEstoque");JTextField textFieldQuantidadeMinimaEstoque = new JTextField(20);
JLabel labelQuantidadeProduto = new JLabel("QuantidadeProduto");JTextField textFieldQuantidadeProduto = new JTextField(20);
JLabel labelStatusIdStatus = new JLabel("StatusIdStatus");JTextField textFieldStatusIdStatus = new JTextField(20);
...
textFieldDataCadastro.setText("");textFieldNomeProduto.setText("");textFieldPrecoUnitarioProduto.setText("");textFieldQuantidadeMinimaEstoque.setText("");textFieldQuantidadeProduto.setText("");textFieldStatusIdStatus.setText("");
}
...
habilitarAtributos(true,false,false,false,false,false,false);btnCreate.setToolTipText("Inserir novo registro");
 ...
 JToolBar Toolbar1 = new JToolBar();
        ...
        centro.setLayout(new GridLayout(7 , 2));centro.add(labelDataCadastro);
...
//-----------------------------  btnRetrieve ------------------------------------------
     btnRetrieve.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent ae) {produto = new Produto();textFieldIdProduto.setText(textFieldIdProduto.getText().trim());//caso tenham sido digitados espaços
...
if (textFieldIdProduto.getText().equals("")) {
 List<String> listaAuxiliar = daoProduto.listInOrderNomeStrings("nome");
   ...
   Point lc = btnRetrieve.getLocationOnScreen();
                    ...
                    textFieldIdProduto.requestFocus();
                    textFieldIdProduto.selectAll();                } else {
 try{
...
textFieldDataCadastro.setText(sdf.format(produto.getDataCadastro()));textFieldNomeProduto.setText(String.valueOf(produto.getNomeProduto()));textFieldPrecoUnitarioProduto.setText(decimalFormat.format(produto.getPrecoUnitarioProduto()));textFieldQuantidadeMinimaEstoque.setText(String.valueOf(produto.getQuantidadeMinimaEstoque()));textFieldQuantidadeProduto.setText(String.valueOf(produto.getQuantidadeProduto()));textFieldStatusIdStatus.setText(String.valueOf(produto.getStatusIdStatus().getIdStatus()+"-"+produto.getStatusIdStatus().getNomeStatus()));atvBotoes(false, true, true, true);
habilitarAtributos(true,false,false,false,false,false,false);                        labelAviso.setText("Encontrou - clic [Pesquisar], [Alterar] ou [Excluir]");
                        acao = "encontrou";
                    } else {
...
produto = new Produto();}
try{
produto.setIdProduto(Integer.valueOf((textFieldIdProduto.getText())));} catch (Exception erro2){ 
 deuRuim = true;
textFieldIdProduto.setBackground(Color.red);
} 
try{
sdf.setLenient(false);
produto.setDataCadastro(sdf.parse((textFieldDataCadastro.getText())));} catch (Exception erro3){ 
 deuRuim = true;
...
produto.setPrecoUnitarioProduto(Double.valueOf((textFieldPrecoUnitarioProduto.getText())));} catch (Exception erro5){ 
 deuRuim = true;
textFieldPrecoUnitarioProduto.setBackground(Color.red);
} 
try{
produto.setQuantidadeMinimaEstoque(Integer.valueOf((textFieldQuantidadeMinimaEstoque.getText())));} catch (Exception erro6){ 
 deuRuim = true;
textFieldQuantidadeMinimaEstoque.setBackground(Color.red);
} 
try{
produto.setQuantidadeProduto(Integer.valueOf((textFieldQuantidadeProduto.getText())));} catch (Exception erro7){ 
 deuRuim = true;
...
daoProduto.inserir(produto);                     labelAviso.setText("Registro inserido.");
} else {
daoProduto.atualizar(produto);                     labelAviso.setText("Registro alterado.");
}
...
labelAviso.setBackground(Color.red);
            ...
            public void actionPerformed(ActionEvent ae) {
                if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(null,
                    ...
                    zerarAtributos();
                     mostrarBotoes(true);
                    atvBotoes(false, true, false, false);                    textFieldDataCadastro.requestFocus();
