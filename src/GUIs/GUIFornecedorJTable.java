...
public class GUIFornecedorJTable extends JDialog {
...
//  ------------------------------------------------------------------------------------------------------ 
  private Container cp;
    ...
    public GUIFornecedorJTable(Point posicao, Dimension dimensao) {

        setTitle("CRUD Fornecedor");
            ...
            public void actionPerformed(ActionEvent evt) {
                if (table.getSelectedRow()>=0) {
                    ...
                    if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(cp,
                            "Confirma a exclusão da Fornecedor [" + tableModel.getValue(table.getSelectedRow()).getIdFornecedor() + " - "
                            + tableModel.getValue(table.getSelectedRow()).getNomeFornecedor() + "]?", "Confirm",
                            JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE)) {
                        btnRem.doClick();
                    }
                } else {
                     JOptionPane.showMessageDialog(cp, "Escolha na tabela a Fornecedor a ser excluída");
                }
            }
        });
...
fornecedor.setIdFornecedor(daoFornecedor.autoIdFornecedor());
fornecedor.setNomeFornecedor("");
                                daoFornecedor.inserir(fornecedor);
                        tableModel.onAdd(fornecedor);
 ...
 btnRem.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    Fornecedor fornecedor = tableModel.getValue(table.getSelectedRow());
 ...
 btnCarregar.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                try {
                    List<Fornecedor> lc = daoFornecedor.list();
...
//============================================ listener table =======================================================
        ...
        tableModel.addTableModelListener(new TableModelListener() {
            ...
            public void tableChanged(TableModelEvent e) {
                // if (tableModel.mudou) {
                if (table.getSelectedRow() != -1 && table.getSelectedRow() < tableModel.getRowCount()) {
                    Fornecedor c = tableModel.getValue(table.getSelectedRow());
                    daoFornecedor.atualizar(c);
                }
                //}
            }
...
public static void main(String[] args) {
        new GUIFornecedorJTable(new Point(880, 250), new Dimension(800, 600));
    }
}
