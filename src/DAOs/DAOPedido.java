...
public class DAOPedido extends DAOGenerico<Pedido> {
    ...
    public List<Pedido> listById(int id) {
        return em.createQuery("SELECT e FROM Pedido + e WHERE e.clienteIdCliente= :id").setParameter("id", id).getResultList();
    }

    public List<Pedido> listInOrderNome() {
        return em.createQuery("SELECT e FROM Pedido e ORDER BY e.clienteIdCliente").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getIdPedido() + "-" + lf.get(i).getClienteIdCliente());
        }
        return ls;
    }
    ...
    public static void main(String[] args) {
        ...
        for (Pedido pedido : listaPedido) {
            System.out.println(pedido.getIdPedido() + "-" + pedido.getClienteIdCliente());
        }
    }
}
