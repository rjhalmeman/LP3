...
public class DAOPedidoHasProduto extends DAOGenerico<PedidoHasProduto> {
    ...
    public int autoQuantidadeProdutoPedido() {
        Integer a = (Integer) em.createQuery("SELECT MAX(e.quantidade) FROM PedidoHasProduto e ").getSingleResult();
        if (a != null) {
            return a + 1;
        } else {
            return 1;
    ...
    public List<PedidoHasProduto> listByNome(String nome) {
        return em.createQuery("SELECT e FROM PedidoHasProduto e WHERE e.quantidade LIKE :nome").setParameter("nome", "%" + nome + "%").getResultList();
    }
    ...
    public List<PedidoHasProduto> listInOrderId() {
        return em.createQuery("SELECT e FROM PedidoHasProduto e ORDER BY e.quantidade").getResultList();
    }
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getQuantidade() + "-" + lf.get(i).getPrecoUnitario());
        }
        return ls;
    }
    ...
    public static void main(String[] args) {
        ...
        for (PedidoHasProduto pedidoHasProduto : listaPedidoHasProduto) {
            System.out.println(pedidoHasProduto.getQuantidade() + "-" + pedidoHasProduto.getPrecoUnitario());
        }
    }
}
